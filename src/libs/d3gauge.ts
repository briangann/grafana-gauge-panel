...
export function drawGauge(svg: any, opt: any) {
  ...
  let defaultFonts = '"Helvetica Neue", Helvetica, Arial, sans-serif';
  ...
  const drawBand = (start: any, end: any, color: any) => {
    if (0 >= end - start) {
      return;
    }
    circleGroup
      ...
      .attr(
        ...
        d3
          ...
          .innerRadius(0.7 * opt.gaugeRadius)
  ...
  function labelXcalc(d: any, i: any) {
    var tickAngle = d + 90,
      tickAngleRad = dToR(tickAngle),
  ...
  function labelYcalc(d: any, i: any) {
    var tickAngle = d + 90,
      tickAngleRad = dToR(tickAngle),
  ...
  needlePath
    ...
    .ease(easeType, 1, 0.9)
    .attrTween('transform', function(d: any, i: any, a: any) {
      let needleAngle = valueScale(opt.needleVal);
      ...
      let needleCentre = originX + ',' + originY;
      let needleRot = needleAngle - opt.zeroNeedleAngle;
  ...
  unitsLabel
    ...
    .tween('text', function(d: any) {
      return () => {
        d.textContent = opt.needleValText;
      };
    });

  // Function to update the gauge value
  // @ts-ignore
  const updateGauge = (newVal: any, newValFormatted: any, newValRounded: any) => {
    ...
    needlePath
      ...
      .ease(easeType, 1, 0.9)
      .attrTween('transform', function(d: any, i: any, a: any) {
        let needleAngleOld = valueScale(oldVal) - opt.zeroNeedleAngle;
        let needleAngleNew = valueScale(newVal) - opt.zeroNeedleAngle;
        //Check for min/max ends of the needle
